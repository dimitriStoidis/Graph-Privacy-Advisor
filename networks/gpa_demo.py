...
class GPA_demo(nn.Module):
    def __init__(self, num_class=2,
        ...
        self.reshape_input = nn.Linear((self._attr_num + 1) * self._num_class,
                                       (self._attr_num + 1))  # self._ggnn_output_channel),  # use for GGNN experiments
    ...
    def forward(self, full_im, categories, card, scene):
        ...
        contextual = Variable(torch.zeros(batch_size, self._graph_num, self._num_class),
                              requires_grad=False).cuda()  # contextual.shape = [bs, 83, 4098]
        if card:
            contextual[:, self._num_class:, 0] = 1.  # size: ([bs,2])
            end_idx = 0
            for b in range(batch_size):
                if categories[0].item() < 12:
                    cur_rois_num = categories[0].item()
                else:
                    cur_rois_num = 12
                end_idx += cur_rois_num
                idxs = categories[1:(cur_rois_num + 1)].data.tolist()
        ...
        if scene:
            scene_logit = resnet_scene(full_im.unsqueeze(0).cuda())
