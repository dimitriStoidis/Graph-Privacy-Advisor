...
def get_metrics(truths, preds):
    ...
    tneg, fpos, fneg, tpos = cm.ravel()
    acc = 100 * (float(tneg) + float(tpos)) / float(tneg + tpos + fneg + fpos)
    pub_prec = tpos / float(tpos + fpos)
    pub_rec = tpos / float(tpos + fneg)
    priv_prec = tneg / float(tneg + fneg)
    priv_rec = tneg / float(tneg + fpos)
    f1_priv = (2*priv_rec*priv_prec)/(priv_prec+priv_rec)
    f1_pub = (2*pub_prec*pub_rec)/(pub_prec+pub_rec)
    macro_f1 = (f1_priv+f1_pub)/2
